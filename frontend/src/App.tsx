    ...
    <div className="App dark-mode theme-default no-reduce-motion">
      <BrowserRouter>
        <Route path="/" exact render={(props) => <>
          { user?.id ? <>
            {user?.is_activated == '1' ? <Dashboard isProfile={false}  user={user}/> : <Ativar user={user} setLogged={setLogged}/>}
          </> : <Login registerSucess={false} setLogged={setLogged} /> }
        </>} />
        {/* Profile route */}
        <Route path="/u/:username/:discrimination" exact render={(props) => <>
          { user?.id ? <>
            {user?.is_activated == '1' ? <Dashboard isProfile={true} user={user}/> : <Ativar user={user} setLogged={setLogged}/>}
          </> : <Login registerSucess={false} setLogged={setLogged} /> }
        </>} />
        {/* End profile route */}
